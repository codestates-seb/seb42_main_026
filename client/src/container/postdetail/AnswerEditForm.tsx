...
const AnswerEditForm: React.FC<Props> = ({ questionId, answerId, value }) => {
  ...
  const editAnswer = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const data = { content: comment };
    if (value.length === 0) return alert('댓글을 입력해주세요');
    try {
      await axios.patch(`${process.env.REACT_APP_BASE_URL}/questions/${questionId}/answers/${answerId}`, data, {
